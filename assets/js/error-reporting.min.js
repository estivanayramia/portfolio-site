(function(){"use strict";const n="site_error_reporting_consent",o="site_error_reporting_asked";function i(){return localStorage.getItem(n)==="true"}function l(){return localStorage.getItem(o)==="true"}function r(e){if(i())try{fetch("/api/error-report",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...e,timestamp:Date.now(),url:location.href,userAgent:navigator.userAgent,viewport:`${window.innerWidth}x${window.innerHeight}`,version:document.querySelector('meta[name="build-version"]')?.content||"unknown"})}).catch(()=>{})}catch{}}function s(){window.addEventListener("error",t=>{r({type:"javascript_error",message:t.message,filename:t.filename,line:t.lineno,col:t.colno,stack:t.error?.stack?.slice(0,1e3)})}),window.addEventListener("unhandledrejection",t=>{r({type:"unhandled_rejection",reason:t.reason?.toString()?.slice(0,500)})});const e=window.fetch;window.fetch=function(...t){return e.apply(this,t).catch(c=>{throw r({type:"network_error",url:t[0],message:c.message}),c})}}function d(){const e=document.createElement("div");e.id="error-reporting-consent",e.innerHTML=`
      <style>
