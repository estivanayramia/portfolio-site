(function(){"use strict";const s=[],c=window.__collectedLogs||[];let b=!1,u="summary",N=!0;function j(e,a){const t={timestamp:Date.now(),type:e,data:a};s.push(t),s.length>1e3&&s.shift(),r()}function x(){const e=document.createElement("div");e.id="debugger-hud",e.innerHTML=`
      <style>
        #debugger-hud {
    ...
    `,document.body.appendChild(e),h(),r()}function h(){const e=document.querySelector(".dbg-pill"),a=document.querySelector(".dbg-panel"),t=document.querySelector(".dbg-close"),n=document.querySelectorAll(".dbg-tab");e.addEventListener("click",o),t.addEventListener("click",o),n.forEach(d=>{d.addEventListener("click",()=>{u=d.dataset.tab,n.forEach(p=>p.classList.remove("active")),d.classList.add("active"),r()})}),document.getElementById("dbg-export").addEventListener("click",S),document.getElementById("dbg-copy").addEventListener("click",D),document.getElementById("dbg-clear").addEventListener("click",T)}function o(){b=!b;const e=document.getElementById("debugger-hud"),a=document.querySelector(".dbg-pill"),t=document.querySelector(".dbg-panel");b?(e.classList.add("expanded"),e.classList.remove("collapsed"),a.style.display="none",t.style.display="flex"):(e.classList.remove("expanded"),e.classList.add("collapsed"),a.style.display="flex",t.style.display="none"),r()}function r(){const e=document.querySelector(".dbg-content");if(!e)return;const a=[...c,...s];switch(u){case"summary":e.innerHTML=v(a);break;case"console":e.innerHTML=y(a);break;case"network":e.innerHTML=w(a);break;case"performance":e.innerHTML=L(a);break;case"layout":e.innerHTML=k();break;case"storage":e.innerHTML=E(a);break}}function v(e){const a=e.filter(n=>n.msg&&n.msg.includes("error")).length,t=e.filter(n=>n.msg&&n.msg.includes("fetch.error")).length;return`
    ...
    `}function y(e){const a=e.filter(t=>t.msg&&t.msg.startsWith("console."));return a.length?a.map(t=>{const n=t.msg.split(".")[1]||"log",d=t.data&&t.data.args?t.data.args.join(" "):"";return`<div class="dbg-log-entry ${n}">[${new Date(t.t).toLocaleTimeString()}] ${d}</div>`}).join(""):"<p>No console logs</p>"}function w(e){const a=e.filter(t=>t.msg&&(t.msg.includes("fetch")||t.msg.includes("xhr")));return a.length?`<table class="dbg-table">
    ...
    </table>`:"<p>No network activity</p>"}function L(e){const a=e.filter(t=>t.msg&&t.msg.startsWith("perf."));return`<p>Performance entries: ${a.length}</p>`+a.slice(0,20).map(t=>`<div class="dbg-log-entry">${t.msg}: ${JSON.stringify(t.data)}</div>`).join("")}function k(){return"<p>Layout scanner coming soon...</p>"}function E(e){return e.filter(t=>t.msg&&(t.msg.includes("Storage")||t.msg.includes("sw."))).map(t=>`<div class="dbg-log-entry">${t.msg}: ${JSON.stringify(t.data)}</div>`).join("")||"<p>No storage activity</p>"}function S(){const e={version:"1.0.0",timestamp:Date.now(),url:location.href,logs:[...c,...s]},a=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),t=URL.createObjectURL(a),n=document.createElement("a");n.href=t,n.download=`debugger-${Date.now()}.json`,n.click(),URL.revokeObjectURL(t)}function D(){const e=JSON.stringify([...c,...s],null,2);navigator.clipboard.writeText(e).then(()=>{alert("Copied to clipboard")}).catch(()=>{alert("Copy failed")})}function T(){s.length=0,r()}let i=[],l=0,g=null;document.addEventListener("touchstart",e=>{if(e.touches.length===3){const a=Date.now();i.push(a),setTimeout(()=>{i.length>=3&&Date.now()-i[0]>=600&&(o(),i=[])},600)}}),document.addEventListener("touchend",()=>{i=[]});const m=document.querySelector('a[href="/"]');m&&m.addEventListener("click",e=>{l++,g&&clearTimeout(g),g=setTimeout(()=>{l=0},500),l===3&&(e.preventDefault(),o(),l=0)}),document.addEventListener("keydown",e=>{e.altKey&&e.shiftKey&&e.key==="D"&&(e.preventDefault(),o())});function f(){console.log("[Debugger] Initializing HUD..."),x(),document.getElementById("debugger-hud").classList.add("collapsed"),console.log("[Debugger] HUD ready. Press Alt+Shift+D to toggle.")}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",f):f()})();
