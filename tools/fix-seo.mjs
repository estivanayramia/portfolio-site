...
function generateRedirects() {
    ...
    for (const [canonical, enFile] of [...canonicalToEnFile.entries()].sort(([, a], [, b]) => a.localeCompare(b))) {
        ...
        const canonicalDestination = legacyRedirectMap.get(canonical) || canonical;
        generatedLines.push(`${enUrlPath}    ${canonicalDestination}    301`);

        // Redirect /EN/foo (no extension) directly to canonical clean URL.
        if (!enFile.endsWith('/index.html')) {
            const enNoExtPath = '/' + enFile.replace(/\.html$/, '');
            generatedLines.push(`${enNoExtPath}    ${canonicalDestination}    301`);
        }
    }
    ...
    const headerLines = [
        ...
        '',
        'https://estivanayramia.com/*    https://www.estivanayramia.com/:splat    301',
        '',
